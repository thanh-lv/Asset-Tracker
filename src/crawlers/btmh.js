...
const API = process.env.BTMH_URL || 'https://baotinmanhhai.vn/api/graphql';
const WANT_CODE = process.env.BTMH_CODE || 'KGB';
...
export async function crawl() {
  const json = await fetchJson(API, {
    method: 'POST',
    headers: { Referer: 'https://baotinmanhhai.vn/', Origin: 'https://baotinmanhhai.vn' },
    body: { query: QUERY, operationName: 'GetGoldRates' }
  });
  const rows = parseBtmhGraphql(json);
  if (rows.length === 0) {
    throw new Error(`Khong tim thay san pham code="${WANT_CODE}" trong goldRates. Kiem tra lai API/code.`);
  }
