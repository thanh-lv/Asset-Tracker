...
function renderPriceGroups(prices) {
  ...
  const names = { BTMH: 'Bảo Tín Mạnh Hải · Vàng', PhuQuy: 'Phú Quý · Bạc', Ancarat: 'Ancarat · Bạc' };
  const order = ['BTMH', 'PhuQuy', 'Ancarat'];

  const wrap = $('#price-groups');
  wrap.innerHTML = '';
  if (prices.length === 0) {
    wrap.innerHTML = '<p class="empty">Chưa có giá. Bấm "Cập nhật giá" để crawl.</p>';
    return;
  }
  for (const src of order) {
    ...
    div.innerHTML = `
      <div style="padding:14px 16px 0">
        <h3 class="pg-title">${names[src] || src} <span class="badge">${rows.length} sản phẩm</span></h3>
      </div>
      <table class="data">
        <thead><tr><th>Sản phẩm</th><th class="num">Mua vào</th><th class="num">Bán ra</th></tr></thead>
        <tbody>
          ${rows
            .map(
              (p) => `<tr class="price-row-link" data-source="${p.source}" data-product="${esc(p.product)}">
                <td>${esc(p.product)}</td>
  ...
  document.querySelectorAll('.price-row-link').forEach((tr) =>
...
async function loadAssets() {
  ...
  for (const a of r.assets) {
    const plClass = a.profitLoss == null ? 'muted' : a.profitLoss >= 0 ? 'up' : 'down';
    const tr = document.createElement('tr');
    tr.innerHTML = `
      <td><span class="tag ${a.type}">${a.type === 'gold' ? 'Vàng' : 'Bạc'}</span></td>
  ...
  if (r.assets.length > 0) {
    const plClass = t.profitLoss == null ? 'muted' : t.profitLoss >= 0 ? 'up' : 'down';
...
function refreshProductOptions() {
  ...
  const sources = [...new Set(LATEST.filter((p) => p.type === type).map((p) => p.source))];
...
function fillProducts() {
  ...
  $('#f-product').innerHTML =
    items.map((p) => `<option value="${esc(p.product)}">${esc(p.product)}</option>`).join('') ||
...
function prefillFromProduct() {
  ...
  if (p) {
    ...
    if (p.sell) $('#f-price').placeholder = 'giá bán ra hiện tại: ' + fmt(p.sell);
  }
}
...
$('#cancel-form').addEventListener('click', () => $('#asset-form').classList.add('hidden'));
...
$('#refresh').addEventListener('click', async () => {
  ...
  try {
    const r = await fetch('/api/crawl', { method: 'POST' }).then((x) => x.json());
...
async function showChart(source, product) {
  const r = await fetch(`/api/prices/history?source=${encodeURIComponent(source)}&product=${encodeURIComponent(product)}`)
    .then((x) => x.json());
...
$('#chart-close').addEventListener('click', () => ($('#chart-block').style.display = 'none'));
...
function drawChart(hist) {
  ...
  for (let g = 0; g <= 3; g++) {
    ...
    const y = Y(v);
    ctx.beginPath(); ctx.moveTo(pad, y); ctx.lineTo(W - pad * 0.4, y); ctx.stroke();
    ctx.fillText(new Intl.NumberFormat('vi-VN').format(Math.round(v)), 4, y + 4 * devicePixelRatio);
  }
  ...
  ctx.beginPath();
  hist.forEach((h, i) => (i ? ctx.lineTo(X(i), Y(h.buy)) : ctx.moveTo(X(i), Y(h.buy))));
  ctx.stroke();
